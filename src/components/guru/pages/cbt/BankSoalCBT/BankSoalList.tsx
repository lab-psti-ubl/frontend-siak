...
type CBTKelas = { id: string; tingkat: number; mataPelajaranId: string; semester: number; tahunAjaran: string };
...
const BankSoalList: React.FC<Props> = ({
  selectedCBTKelas,
  bankSoal,
  tingkatLabel,
  getMapelName,
        ...
        <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h2 className="text-base sm:text-lg font-semibold text-slate-900">
              Bank Soal CBT • {getMapelName(selectedCBTKelas.mataPelajaranId)}
            </h2>
            <p className="text-xs sm:text-sm text-slate-500">
              {tingkatLabel(selectedCBTKelas.tingkat)} • Semester {selectedCBTKelas.semester} • Tahun Ajaran {selectedCBTKelas.tahunAjaran}
            </p>
          </div>
        ...
        {bankSoal.length === 0 ? (
          <div className="py-10 sm:py-12 px-5 sm:px-6 text-center">
            ...
            <p className="text-xs sm:text-sm text-slate-500 max-w-md mx-auto">
              ...
              atas untuk membuat kelompok bank soal berdasarkan kategori nilai dan
              jenis soal.
            </p>
          </div>
        ) : (
