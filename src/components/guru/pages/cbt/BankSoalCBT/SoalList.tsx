...
type CBTKelas = { tingkat: number; mataPelajaranId: string };
type CBTBankSoal = { id: string; judul: string; kategoriNama: string; tipe: string };

type Props = {
  selectedCBTKelas: CBTKelas;
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="flex items-center justify-between gap-2">
        <Button variant="secondary" size="sm" className="flex items-center gap-1" onClick={onBack}>
        ...
        <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500">
              {tingkatLabel(selectedCBTKelas.tingkat)} • {getMapelName(selectedCBTKelas.mataPelajaranId)} • Kategori {selectedBank.kategoriNama} • Jenis {selectedBank.tipe.replace(/_/g, ' ')}
            </p>
          </div>
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <Button size="sm" variant="secondary" className="p-1.5 text-blue-600 hover:bg-blue-50" onClick={() => onDetail(item)}>
                        <Eye className="w-4 h-4" />
                      </Button>
                      <Button size="sm" variant="secondary" className="p-1.5 text-amber-600 hover:bg-amber-50" onClick={() => onEdit(item)}>
                        <Pencil className="w-4 h-4" />
                      </Button>
                      <Button size="sm" variant="secondary" className="p-1.5 text-red-600 hover:bg-red-50" onClick={() => onDelete(item)}>
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    </div>
                  </div>
                  <p className="text-xs sm:text-sm text-slate-800 line-clamp-3">{item.pertanyaan}</p>
                </div>
              ))}
            </div>
          </div>
