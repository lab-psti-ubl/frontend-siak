...
const JadwalTahfizGuru: React.FC = () => {
  const { user } = useAuth();
  const { t } = useLanguage();
  ...
  // Filter jadwal to only show schedules for classes where the logged-in guru is the ustadz
  ...
  }, [jadwalTahfiz, myClasses, user?.id]);
  ...
  const enrichedJadwal = useMemo(() => {
    return myJadwal.map((item) => {
      ...
      const ustadzName = kelas ? ustadzMap.get(kelas.ustadzId)?.name || 'Belum diatur' : 'Belum diatur';
      return {
        ...item,
        kelas,
        ustadzName,
  ...
  }, [sortedJadwal, searchTerm, dayOptions]);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-1 sm:mb-2">
                {t('tahfiz.guruTahfiz.jadwalTahfizGuru.title')}
              </h1>
              <p className="text-sm sm:text-base text-emerald-100">
                {t('tahfiz.guruTahfiz.jadwalTahfizGuru.subtitle')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 lg:p-6 space-y-3 sm:space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder={t('tahfiz.guruTahfiz.jadwalTahfizGuru.cariKelasRuanganUstadzHari')}
                value={searchTerm}
          ...
          <div className="text-xs sm:text-sm text-slate-600">
            {t('tahfiz.guruTahfiz.jadwalTahfizGuru.menampilkan')} <span className="font-semibold text-slate-900">{filteredJadwal.length}</span> {t('tahfiz.guruTahfiz.jadwalTahfizGuru.dari')}{' '}
            <span className="font-semibold text-slate-900">{myJadwal.length}</span> {t('tahfiz.guruTahfiz.jadwalTahfizGuru.jadwal')}
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="text-center py-12 px-6">
            ...
            <p className="text-sm text-slate-600">{t('tahfiz.guruTahfiz.jadwalTahfizGuru.memuatDataJadwal')}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="text-center py-12 px-6">
            ...
            <h3 className="text-lg font-medium text-slate-900 mb-2">
              {searchTerm ? t('tahfiz.guruTahfiz.jadwalTahfizGuru.tidakAdaHasil') : t('tahfiz.guruTahfiz.jadwalTahfizGuru.belumAdaJadwal')}
            </h3>
            <p className="text-sm text-slate-600 mb-4">
              {searchTerm ? t('tahfiz.guruTahfiz.jadwalTahfizGuru.ubahKataKunci') : t('tahfiz.guruTahfiz.jadwalTahfizGuru.belumMemilikiJadwal')}
            </p>
          </div>
        </div>
      ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        ...
                        {isToday && (
                          <p className="text-xs text-blue-100">{t('tahfiz.guruTahfiz.jadwalTahfizGuru.hariIni')}</p>
                        )}
                      </div>
                    </div>
                    <Badge variant="default" size="sm">
                      {schedules.length} {t('tahfiz.guruTahfiz.jadwalTahfizGuru.jadwal')}
                    </Badge>
                  </div>
                </div>
                ...
                <div className="p-4 sm:p-5 lg:p-6">
                  {schedules.length > 0 ? (
                      ...
                      {schedules.map((item) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              if (isToday) {
                                navigate('/dashboard/absensi-tahfiz', { state: { scrollToJadwalId: item.id } });
                              }
                            }}
                            className={`group relative bg-gradient-to-br from-slate-50 to-slate-50 hover:from-emerald-50 hover:to-emerald-50 border border-slate-200 rounded-lg sm:rounded-xl p-3 sm:p-4 transition-all duration-200 ${
                              isToday ? 'cursor-pointer hover:shadow-md hover:border-emerald-300' : ''
                            }`}
                          >
                    ...
                    <div className="text-center py-6 sm:py-8">
                      ...
                      <p className="text-xs sm:text-sm font-medium text-slate-500">{t('tahfiz.guruTahfiz.jadwalTahfizGuru.tidakAdaJadwalHari')}</p>
                    </div>
                  )}
                </div>
              </div>
...
export default JadwalTahfizGuru;
