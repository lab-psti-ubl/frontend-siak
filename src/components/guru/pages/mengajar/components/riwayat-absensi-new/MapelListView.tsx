...
interface MapelListViewProps {
  ...
  onViewPertemuan: (kelasId: string, mapelId: string, jadwalId: string) => void;
  onViewRekap: (kelasId: string, mapelId: string, jadwalId: string) => void;
...
const MapelListView: React.FC<MapelListViewProps> = ({
  ...
  // This ensures only relevant schedules for the teacher are shown
  const kelasSchedules = useMemo(() => {
    let filtered = mySchedules.filter(s => s.kelasId === kelasId);
    
    // Additional filter by tahun ajaran and semester if provided
    ...
    return filtered;
  }, [mySchedules, kelasId, selectedTahunAjaran, selectedSemester]);
        ...
        <div className="p-4 sm:p-6 lg:p-8 flex items-center gap-4 sm:gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs sm:text-sm text-slate-600 mt-1">
              {kelasSchedules.length} mata pelajaran yang diajar
            </p>
          </div>
        </div>
      </div>
                  ...
                  <tr key={jadwal.id} className="hover:bg-slate-50 transition-colors duration-150">
                    ...
                    <td className="px-5 sm:px-6 py-3 sm:py-4">
                      <div className="flex items-center gap-2">
                        <div className="flex-shrink-0 h-8 w-8 bg-gradient-to-br from-violet-400 to-pink-500 rounded-lg flex items-center justify-center shadow-sm">
                    ...
                    <td className="px-5 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <Button
                          onClick={() => onViewPertemuan(kelasId, mapel?.id || '', jadwal.id)}
                          variant="primary"
                        ...
                        <Button
                          onClick={() => onViewRekap(kelasId, mapel?.id || '', jadwal.id)}
                          variant="secondary"
          ...
          return (
            <div key={jadwal.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
              <div className="p-4">
                ...
                <div className="space-y-2.5 mb-4 pb-4 border-b border-slate-200">
