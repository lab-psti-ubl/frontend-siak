...
const VerificationPage: React.FC<VerificationPageProps> = ({
  ...
  const docTypeLabel = useMemo(() => {
    const key = `verificationPage.docType.${documentType}`;
    const label = t(key);
    return label === key ? t('verificationPage.docType.default') : label;
  }, [documentType, t]);

  const docTypeDetailLabel = useMemo(() => {
    const key = `verificationPage.docTypeDetail.${documentType}`;
    const label = t(key);
    return label === key ? t('verificationPage.docTypeDetail.default') : label;
  }, [documentType, t]);
    ...
    if (element) {
      ...
      if (printWindow) {
        printWindow.document.write(`
          ...
          <head>
            <meta charset="utf-8">
            <title>${docTypeLabel} - ${t('verificationPage.titlePrint')}</title>
            <style>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-600">
                {t('verificationPage.subtitle')} {schoolData.namaSekolah}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden sticky top-24 max-h-[calc(100vh-150px)] overflow-y-auto">
              ...
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 border-b-2 border-green-200 p-6">
                ...
                <h2 className="text-2xl font-bold text-center text-slate-900 mb-2">
                  {t('verificationPage.verifikasiBerhasil')}
                </h2>
                <p className="text-center text-sm text-slate-700">
                  {docTypeDetailLabel} - {t('verificationPage.telahDitandatangani')}
                </p>
              </div>
              ...
              <div className="p-6">
                <div className="space-y-1 mb-6">
                  <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider">
                    {t('verificationPage.detailVerifikasi')}
                  </h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between py-3 border-b border-slate-200">
                    <span className="text-sm font-medium text-slate-600">
                      {t('verificationPage.jenisDokumen')}
                    </span>
                  ...
                  {userName && (
                    <div className="flex items-start justify-between py-3 border-b border-slate-200">
                      <span className="text-sm font-medium text-slate-600">{t('verificationPage.nama')}</span>
                  ...
                  <div className="flex items-start justify-between py-3 border-b border-slate-200">
                    <span className="text-sm font-medium text-slate-600">
                      {t('verificationPage.waktuVerifikasi')}
                    </span>
                  ...
                  <div className="flex items-start justify-between py-3 border-b border-slate-200">
                    <span className="text-sm font-medium text-slate-600">
                      {t('verificationPage.metodeVerifikasi')}
                    </span>
                  ...
                  <div className="flex items-center justify-between py-3 bg-green-50 rounded-lg px-3 border border-green-200">
                    <span className="text-sm font-medium text-slate-600">
                      {t('verificationPage.status')}
                    </span>
                    <div className="flex items-center gap-2">
                      <div className="w-2.5 h-2.5 rounded-full bg-green-600"></div>
                      <span className="text-sm font-bold text-green-700">
                        {t('verificationPage.sahTerverifikasi')}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-r-lg">
                  <p className="text-xs text-blue-900 leading-relaxed">
                    <strong>{t('verificationPage.informasi')}:</strong>{' '}
                    {t('verificationPage.informasiDesc')}
                  </p>
                </div>
                ...
                {signatureTitle && (
                  <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg text-center">
                    <p className="text-xs text-amber-900">
                      {t('verificationPage.ditandatanganiOleh')}
                    </p>
              ...
              <div className="border-t border-slate-200 p-6 bg-slate-50 space-y-3">
                ...
                >
                  ...
                  <span>{t('verificationPage.cetakHalaman')}</span>
                </button>
              ...
              <div className="bg-slate-100 border-t border-slate-200 px-6 py-4 text-center">
                <p className="text-xs text-slate-600">
                  {t('verificationPage.footerDigital')}
                  {signatureTitle ? t('verificationPage.footerOleh') + signatureTitle : t('verificationPage.footerOlehSistem')}
                  {t('verificationPage.footerSah')}
                </p>
                <p className="text-xs text-slate-600 mt-2">
                  © {new Date().getFullYear()} {t('verificationPage.sistemManajemenSekolah')}
                </p>
              </div>
            </div>
          </div>
            ...
            <div id="verification-content" className="bg-white rounded-2xl shadow-lg overflow-hidden">
              {documentContent ? (
                ...
                <div className="p-8 sm:p-12 text-center min-h-[400px] flex flex-col items-center justify-center">
                  ...
                  <p className="text-slate-600 font-medium">{t('verificationPage.kontenDokumen')}</p>
                  <p className="text-slate-500 text-sm mt-1">
                    {t('verificationPage.kontenDokumenDesc')}
                  </p>
                </div>
              )}
            </div>
            ...
            {isMobile && (
              <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200">
                <h3 className="font-semibold text-slate-900 mb-3">
                  {t('verificationPage.informasiVerifikasi')}
                </h3>
                <div className="space-y-2 text-sm text-slate-700">
                  <p>
                    {t('verificationPage.status')}:{' '}
                    <strong className="text-green-600">{t('verificationPage.sahTerverifikasi')}</strong>
                  </p>
                  <p>
                    {t('verificationPage.jenis')}: <strong>{docTypeLabel}</strong>
                  </p>
                  <p>
                    ID: <strong className="font-mono text-xs break-all">{suratId}</strong>
                  </p>
                  <p>
                    {t('verificationPage.waktu')}: <strong>{timestamp}</strong>
                  </p>
                </div>
              </div>
            )}
