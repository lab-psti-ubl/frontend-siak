...
import {
  ...
  GuruImportData,
  checkDuplicates
} from '../../../../utils/excelGuruImport';
import { User } from '../../../../types';

interface ExcelImportModalGuruProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (data: GuruImportData[]) => void;
  existingUsers: User[];
}

const ExcelImportModalGuru: React.FC<ExcelImportModalGuruProps> = ({
  isOpen,
  onClose,
  onImport,
  existingUsers
  ...
  const [previewData, setPreviewData] = useState<GuruImportData[] | null>(null);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Import Data Guru dari Excel">
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              ...
              <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
                ...
                <li>Kolom wajib: nama, email, nip</li>
                <li>Kolom opsional: phone, posisi (Guru/TU/Staff/Magang)</li>
        ...
        <div className="space-y-4">
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
            <input
              ...
              className="hidden"
              id="excel-file-input-guru"
            />
            <label htmlFor="excel-file-input-guru" className="cursor-pointer">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-emerald-800">
                    Ditemukan {previewData.length} guru yang siap diimport
                  </p>
                </div>
              </div>
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="bg-emerald-100 sticky top-0">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-left text-emerald-900 font-medium whitespace-nowrap">NIP</th>
                        <th className="px-4 py-2 text-left text-emerald-900 font-medium whitespace-nowrap">Posisi</th>
                      </tr>
                    </thead>
                    <tbody className="text-emerald-800">
                      {previewData.slice(0, 10).map((guru, index) => (
                        <tr key={index} className="border-t border-emerald-100 hover:bg-emerald-50">
                          <td className="px-4 py-2 whitespace-nowrap">{index + 1}</td>
                          <td className="px-4 py-2 whitespace-nowrap">{guru.nama}</td>
                          <td className="px-4 py-2 whitespace-nowrap">{guru.email}</td>
                          <td className="px-4 py-2 whitespace-nowrap">{guru.nip}</td>
                          <td className="px-4 py-2 whitespace-nowrap">{guru.subject || '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {previewData.length > 10 && (
                  <div className="px-4 py-2 bg-emerald-50 border-t border-emerald-200 text-center">
                    <p className="text-xs text-emerald-700">
                      ... dan {previewData.length - 10} guru lainnya
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="flex space-x-3">
          ...
          {previewData && previewData.length > 0 && (
            ...
            >
              ...
              Import {previewData.length} Guru
            </Button>
          )}
        </div>
      </div>
...
export default ExcelImportModalGuru;
