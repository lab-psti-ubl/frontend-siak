...
interface RFIDTokenAuthProps {
  onAuthenticate: (isValid: boolean) => void;
  expectedToken: string;
  currentAlat?: AlatRFID | null;
}

const RFIDTokenAuth: React.FC<RFIDTokenAuthProps> = ({ onAuthenticate, expectedToken, currentAlat }) => {
  ...
  useEffect(() => {
    if (token && expectedToken && token === expectedToken && !isLoading) {
      setTimeout(() => {
  ...
  }, [token, expectedToken, isLoading, currentAlat, onAuthenticate]);
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="Token akan otomatis terisi..."
