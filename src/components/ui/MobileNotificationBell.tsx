...
import { Bell, X, Calendar, GraduationCap, FileText, Users } from 'lucide-react';
...
const MobileNotificationBell: React.FC = () => {
  ...
  // Generate notifications (same logic as desktop version)
  useEffect(() => {
    if (!user) return;
    ...
    relevantInfoSekolah.forEach(info => {
      newNotifications.push({
        id: `info-${info.id}`,
        title: info.judul,
        message: info.konten.substring(0, 80) + (info.konten.length > 80 ? '...' : ''),
      ...
      if (user.role === 'murid') {
        ...
        if (myKelas && isMaxTingkat(myKelas.tingkat)) {
          newNotifications.push({
            ...
            message: `Pengumuman kelulusan tahun ajaran ${activeKelulusan.tahunAjaran} telah dipublikasikan.`,
        ...
        if (myKelas && isMaxTingkat(myKelas.tingkat)) {
          newNotifications.push({
            ...
            message: `Pengumuman kelulusan untuk kelas ${myKelas.name} telah dipublikasikan.`,
      ...
      if (myKelas && !isMaxTingkat(myKelas.tingkat)) {
        ...
        if (statusKenaikan) {
          newNotifications.push({
            ...
            title: 'Siap Sebarkan Raport',
            message: `Raport kenaikan kelas untuk ${myKelas.name} siap disebarkan.`,
      ...
      if (myKelas) {
        ...
        if (statusBagiRaportData) {
          newNotifications.push({
            ...
            title: 'Siap Sebarkan Raport',
            message: `Raport semester ganjil untuk ${myKelas.name} siap disebarkan.`,
        ...
        if (activeTahunAjaran.semester === 2 && !isMaxTingkat(myKelas.tingkat)) {
          ...
          if (statusKenaikan) {
            newNotifications.push({
              ...
              title: 'Raport Tersedia',
              message: `Raport kenaikan kelas untuk ${myKelas.name} telah disebarkan.`,
        ...
        if (activeTahunAjaran.semester === 1) {
          ...
          if (statusBagiRaportData) {
            newNotifications.push({
              ...
              title: 'Raport Tersedia',
              message: `Raport semester ganjil untuk ${myKelas.name} telah disebarkan.`,
  ...
  useEffect(() => {
    ...
    // 2. It is unread
    // 3. It hasn't been auto-opened in this session (mobile)
    if (newestNotification && !newestNotification.isRead) {
      // Check if this notification has already been auto-opened in this session (mobile)
      const autoOpenedNotifications = sessionStorage.getItem('autoOpenedNotificationsMobile');
      const openedIds = autoOpenedNotifications ? JSON.parse(autoOpenedNotifications) : [];
      
      if (!openedIds.includes(newestNotification.id)) {
        // Mark as auto-opened for mobile
        openedIds.push(newestNotification.id);
        sessionStorage.setItem('autoOpenedNotificationsMobile', JSON.stringify(openedIds));
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.mobile-notification-dropdown')) {
        setIsOpen(false);
      }
    };
