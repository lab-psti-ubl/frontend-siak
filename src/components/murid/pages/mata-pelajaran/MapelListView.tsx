...
import { BookOpen, Clock, User, Eye, FileText, FileSpreadsheet } from 'lucide-react';
import { JadwalPelajaran, MataPelajaran, User as UserType, Kelas, SesiAbsensi } from '../../../../types';
import MateriMuridModal from './MateriMuridModal';
...
const MapelListView: React.FC<MapelListViewProps> = ({
        ...
        <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl sm:rounded-2xl border border-blue-200 shadow-sm overflow-hidden">
          <div className="p-4 sm:p-6 lg:p-8 flex items-center gap-4 sm:gap-6">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-base sm:text-lg lg:text-xl font-bold text-slate-900">{kelas?.name || 'Kelas Anda'}</h3>
              <p className="text-xs sm:text-sm text-slate-600 mt-1">Belum ada mata pelajaran untuk periode ini</p>
            </div>
            <Button
              onClick={onViewRekapAbsen}
              className="flex items-center text-xs sm:text-sm px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg"
            >
              <FileSpreadsheet size={14} className="sm:mr-2" />
              <span className="hidden sm:inline">Rekap Absen</span>
            </Button>
          </div>
        </div>
        <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex flex-col items-center justify-center py-12 sm:py-16 px-4 sm:px-6">
            ...
            <p className="text-xs sm:text-sm text-slate-600 text-center">
              Belum ada jadwal mata pelajaran untuk periode ini
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-5 lg:space-y-6">
      <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl sm:rounded-2xl border border-blue-200 shadow-sm overflow-hidden">
        <div className="p-4 sm:p-6 lg:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
          <div className="flex items-center gap-4 sm:gap-6 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm text-slate-600 mt-1">
                {mySchedules.length} mata pelajaran tersedia
              </p>
            </div>
          </div>
          <Button
            onClick={onViewRekapAbsen}
            className="flex items-center text-xs sm:text-sm px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg w-full sm:w-auto justify-center"
          >
                  ...
                  <tr key={jadwal.id} className="hover:bg-slate-50 transition-colors duration-150">
                    ...
                    <td className="px-5 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                      {mapel?.keterangan === 'umum' ? (
                        ...
                        <Badge variant="success" className="text-xs">Agama</Badge>
                      ) : (
          ...
          return (
            <div key={jadwal.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
              <div className="p-4">
                ...
                <div className="flex flex-row space-y-2.5 mb-4 pb-4 border-b border-slate-200">
