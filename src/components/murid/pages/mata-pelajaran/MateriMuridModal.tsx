...
import { SesiAbsensi, JadwalPelajaran, MataPelajaran, TahunAjaran } from '../../../../types';
import { useTahunAjaran } from '../../../../hooks/useTahunAjaran';
import { useJurnal, Jurnal } from '../../../../hooks/useJurnal';

interface MateriMuridModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const MateriMuridModal: React.FC<MateriMuridModalProps> = ({
  isOpen,
  onClose,
  selectedSchedules,
  mataPelajaran,
            ...
            <div className="p-4 sm:p-5 lg:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 pt-4 sm:pt-5 border-t border-blue-200">
                ...
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <Badge variant="info" size="sm">Ada Pertemuan</Badge>
                </div>
              </div>
            </div>
          </div>
                      ...
                      {sesiWithMateri.map((meeting, index) => (
                        <tr key={`${meeting.tanggal}-${index}`} className="hover:bg-slate-50 transition-colors duration-150">
                          ...
                          <td className="px-4 sm:px-5 py-3 sm:py-4">
                            {meeting.sesi ? (
                              meeting.isVirtual ? (
                                <Badge variant="warning" size="sm">Virtual</Badge>
                              ) : (
                ...
                {sesiWithMateri.map((meeting, index) => (
                  <div key={`${meeting.tanggal}-${index}`} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                    <div className="p-4">
                      ...
                      <div className="flex flex-row space-y-2.5 mb-3 pb-3 border-b border-slate-200">
        ...
        >
          <div className="space-y-4 sm:space-y-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs sm:text-sm text-slate-600 mt-1">{mataPelajaran?.name}</p>
                    </div>
                  </div>
                  <Badge variant="info" size="sm">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 sm:p-5">
              <h5 className="text-sm sm:text-base font-semibold text-slate-900 mb-3 flex items-center">
                <FileText className="w-4 h-4 mr-2 text-slate-600" />
                Deskripsi Materi
              </h5>
              <div className="text-slate-700 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap bg-slate-50 rounded-lg p-3 sm:p-4 border border-slate-200">
            ...
            {selectedMateri.jurnal.file && (
              <div className="bg-white border border-slate-200 rounded-xl p-4 sm:p-5">
                ...
                <div className="bg-gradient-to-r from-slate-50 to-slate-100 border border-slate-200 rounded-lg p-3 sm:p-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="flex-shrink-0 h-10 w-10 sm:h-12 sm:w-12 bg-white rounded-lg flex items-center justify-center shadow-sm border border-slate-200">
...
export default MateriMuridModal;
