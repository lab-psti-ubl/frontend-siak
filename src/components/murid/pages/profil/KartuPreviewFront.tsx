...
const KartuPreviewFront: React.FC<KartuPreviewFrontProps> = ({ user, myKelas, myJurusan, backgroundImage, orientation = 'potrait' }) => {
  ...
  const normalizedQRCode = normalizeStudentQRCode(user.qrCode, user.nisn);
          ...
          >
            ...
            <div className="relative z-10 flex-1 text-left text-black" style={{
              ...
              <div className={`${values.gap}`} style={{
                ...
                <span>:</span>
                <span className="truncate">{user?.nisn || '-'}</span>

                {showJurusan && myJurusan && (
                  <>
                    <span className="font-medium">Jurusan</span>
                    <span>:</span>
                    <span className="truncate">{myJurusan?.name || '-'}</span>
                  </>
                )}

                <span className="font-medium">Telp. Ortu</span>
                <span>:</span>
                <span className="truncate">{user?.whatsappOrtu || '-'}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex justify-center overflow-x-auto pb-2">
        ...
        >
          ...
          <div className="relative z-10 text-center flex-shrink-0 w-full px-2 text-black">
            ...
            <div className={`space-y-0.5 text-left ${values.textSize}`} style={{
              lineHeight: 1.2,
              display: 'grid',
              gridTemplateColumns: 'auto auto 1fr',
              columnGap: '4px',
              rowGap: '2px',
              fontSize: (values.textSize as string) === 'text-xs' ? '16px' : (values.textSize as string) === 'text-[11px]' ? '11px' : '9px',
              maxWidth: '100%',
            }}>
              <span className="font-medium">NISN</span>
              <span>:</span>
...
export default KartuPreviewFront;
