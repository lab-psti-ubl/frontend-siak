...
import { CheckCircle2, BookOpen, ArrowLeft, User, Calendar, X } from 'lucide-react';
...
interface HasilTesModalProps {
  isOpen: boolean;
  onClose: () => void;
  progress: ProgressHafalan;
}

const HasilTesModal: React.FC<HasilTesModalProps> = ({
  isOpen,
  onClose,
  progress,
}) => {
  ...
  const getHasilTesInfo = () => {
    ...
    const info = {
      'Mumtaz': {
        ...
        arabic: 'ممتاز',
        keputusan: t('tahfiz.muridTahfiz.hasilTesModal.bolehLanjut'),
        catatan: t('tahfiz.muridTahfiz.hasilTesModal.catatanMumtaz').replace('{surat}', progress.surat).replace('{ayatDari}', progress.ayatDari.toString()).replace('{ayatSampai}', progress.ayatSampai.toString()),
      },
      'Jayid Jiddan': {
        bgColor: 'bg-blue-50',
        ...
        arabic: 'جيد جدًا',
        keputusan: t('tahfiz.muridTahfiz.hasilTesModal.bolehLanjutDenganMurajaah'),
        catatan: t('tahfiz.muridTahfiz.hasilTesModal.catatanJayidJiddan').replace('{surat}', progress.surat).replace('{ayatDari}', progress.ayatDari.toString()).replace('{ayatSampai}', progress.ayatSampai.toString()),
      },
    };

    return info[progress.hasilTes as 'Mumtaz' | 'Jayid Jiddan'];
  };
    ...
    const fetchSurahData = async () => {
      ...
      try {
        ...
        if (!surahListData.data) {
          setError(t('tahfiz.muridTahfiz.hasilTesModal.gagalMengambilDataSurah'));
          return;
        }
        ...
        if (!surah) {
          setError(t('tahfiz.muridTahfiz.hasilTesModal.suratTidakDitemukan'));
          return;
        }
        ...
        if (data.data && data.data.ayat) {
          ...
          setError(t('tahfiz.muridTahfiz.hasilTesModal.dataAyatTidakDitemukan'));
        }
      } catch (err) {
        console.error('Error fetching surah data:', err);
        setError(t('tahfiz.muridTahfiz.hasilTesModal.gagalMemuatDataAyat'));
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="relative px-5 sm:px-6 lg:px-8 py-6 sm:py-8">
            <div className="flex items-center gap-4 mb-6">
              <Button
                ...
                title={t('tahfiz.muridTahfiz.hasilTesModal.tutup')}
              >
                <ArrowLeft size={20} />
              </Button>
              <div className="flex-1">
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-2 drop-shadow-lg flex items-center gap-2">
                  ...
                  {t('tahfiz.muridTahfiz.hasilTesModal.title')}
                </h1>
          ...
          <div className="p-4 sm:p-6 lg:p-8 space-y-4">
            ...
            {hasilTesInfo && (
              <div className={`${hasilTesInfo.bgColor} border-2 ${hasilTesInfo.borderColor} rounded-xl p-5 shadow-lg`}>
                <div className="flex items-start gap-3">
                  <CheckCircle2 className={`w-7 h-7 ${hasilTesInfo.textColor} flex-shrink-0 mt-1`} />
                  <div className="flex-1">
            ...
            {loading && (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-base text-emerald-800 font-medium mt-6">{t('tahfiz.muridTahfiz.hasilTesModal.memuatDataAyat')}</p>
              </div>
            )}
