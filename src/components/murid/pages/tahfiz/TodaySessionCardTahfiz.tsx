...
import { Clock, User, CheckCircle, QrCode } from 'lucide-react';
import Button from '../../../ui/Button';
import Badge from '../../../ui/Badge';
import { SesiAbsensiTahfiz, AbsensiPelajaran } from '../../../../types';
import { useLanguage } from '../../../../context/LanguageContext';

interface TodaySessionCardTahfizProps {
  sesi: SesiAbsensiTahfiz;
  mapel: string;
  ustadz: string;
  waktu: string;
  kelas: string;
  attendance?: AbsensiPelajaran;
  onScanQR: () => void;
}

const TodaySessionCardTahfiz: React.FC<TodaySessionCardTahfizProps> = ({
  sesi,
  mapel,
  ustadz,
  waktu,
        ...
        <div className="flex-1">
          <div className="flex items-start gap-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center mt-1.5 text-xs sm:text-sm text-slate-600">
                ...
                <span className="truncate">{ustadz}</span>
              </div>
              <div className="text-xs text-slate-500 mt-1">
                {t('tahfiz.muridTahfiz.todaySessionCard.kelas')}: {kelas}
              </div>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2 ml-8">
            ...
            <div className="text-xs text-slate-600">
              {t('tahfiz.muridTahfiz.todaySessionCard.dibuka')}: {sesi.jamBuka}
              {sesi.jamTutup && ` - ${t('tahfiz.muridTahfiz.todaySessionCard.ditutup')}: ${sesi.jamTutup}`}
            </div>
          </div>
        </div>
        <div className="flex items-center justify-end sm:justify-start">
          {attendance ? (
            ...
            >
              ...
              <span className="hidden sm:inline">{t('tahfiz.muridTahfiz.todaySessionCard.scanQR')}</span>
              <span className="sm:hidden">{t('tahfiz.muridTahfiz.todaySessionCard.scan')}</span>
            </Button>
          ) : (
            <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-slate-200 text-slate-700 opacity-60">
              {t('tahfiz.muridTahfiz.todaySessionCard.sesiDitutup')}
            </span>
          )}
        </div>
      </div>
...
export default TodaySessionCardTahfiz;
